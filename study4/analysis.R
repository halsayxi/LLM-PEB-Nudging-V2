...
human_data_path <- file.path(script_dir, "data", "data_human_deidentified.csv")
output_dir <- file.path(script_dir, "outputs")
dir.create(output_dir, recursive = TRUE, showWarnings = FALSE)

if (!file.exists(human_data_path)) {
  stop("data/data_human_deidentified.csv was not found relative to the script.", call. = FALSE)
}
...
figure3_panel_a <- main_trend_plot + common_composite_theme +
  theme(legend.position = c(0.01, 0.98), plot.tag.position = c(0.002, 0.998))
figure3_panel_b <- donation_hurdle_plot + common_composite_theme +
  theme(strip.background = element_blank(), panel.border = element_blank(), axis.line = element_line(color = "#222222", linewidth = 0.45), plot.tag.position = c(0.002, 0.998))
figure3_panel_c <- donation_event_window_plot + common_composite_theme +
  theme(panel.border = element_blank(), axis.line = element_line(color = "#222222", linewidth = 0.45), legend.position = c(0.98, 0.98), plot.tag.position = c(0.002, 0.998))
figure3_panel_d <- attitude_feedback_plot + common_composite_theme +
  theme(axis.title.x = element_blank(), axis.text.x = element_text(size = 7.5), plot.tag.position = c(0.002, 0.965))
figure3_panel_e <- motivation_eco_change_plot + common_composite_theme +
  theme(plot.tag.position = c(0.002, 0.965))

figure3_row_bc <- figure3_panel_b + figure3_panel_c + plot_layout(widths = c(0.62, 0.38))
figure3_row_de <- figure3_panel_d + figure3_panel_e + plot_layout(widths = c(0.72, 0.28))
figure3_plot <- figure3_panel_a / figure3_row_bc / figure3_row_de +
  plot_layout(heights = c(0.92, 0.86, 1.16)) +
  plot_annotation(tag_levels = "a") &
  theme(plot.tag = element_text(face = "bold", size = 15, color = "#111111"), plot.margin = margin(3, 4, 3, 4))
...
save_png(figure3_plot, "figure3_human_study.png", width = 18, height = 13.6, dpi = 400)
